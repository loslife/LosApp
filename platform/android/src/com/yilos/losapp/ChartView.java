...
public class ChartView extends View 
{
	...
	public int XLength = 550; // X轴的长度
	public int YLength = 600; // Y轴的长度
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 1; i * YScale <= YLength; i++) {
			canvas.drawLine(XPoint, YPoint - YLength +i * YScale, XPoint + 5, YPoint - YLength +i * YScale, paint); // 刻度 XPoint+5画出了一条短的小横线
			try {
				canvas.drawText(XLabel[i], XPoint - 90,
						YPoint - YLength + i * YScale + 5, paint1); // 文字
			} catch (Exception e) {
			}
		}
		...
		// 设置X轴
		canvas.drawLine(XPoint, YPoint- YLength, XPoint + XLength, YPoint- YLength, paint); // 轴线
		for (int i = 0; i * XScale < XLength; i++) {
			/*canvas.drawLine(XPoint + i * XScale, YPoint-YLength, XPoint + i * XScale,
					YPoint- YLength- 5, paint); // 刻度
*/			try {
				canvas.drawText(YLabel[i], XPoint + i * XScale - 5,
						YPoint- YLength, paint1); // 文字
				// 数据值
				if (i > 0 && YCoord(Data[i - 1]) != -999
						&& YCoord(Data[i]) != -999) // 保证有效数据
					canvas.drawLine(YCoord(Data[i - 1]),
							YPoint- YLength + i * YScale, YCoord(Data[i]),
							YPoint- YLength + (i+1) * YScale, paint);
				canvas.drawCircle(YCoord(Data[i]),YPoint- YLength + (i+1) * YScale , 4,
						paint);
				canvas.drawText(Data[i]+"人", YCoord(Data[i])+10,YPoint- YLength + (i+1) * YScale, paint1); // 文字
			} catch (Exception e) {
			}
		}
